...
class Pointillism(Filter):
    background = 'pointillism_bg.jpg'

    def filter(self, frame: MatLike):
        # setup parameters
        stroke_scale = int(math.ceil(max(frame.shape) / 1000))
        gradient_smoothing_radius = int(round(max(frame.shape) / 50))

        # convert the image to grayscale to compute the gradient
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)

        # Compute color palette
        palette = ColorPalette.from_image(frame, 20) # change second arg to how many colors to use
        ...
        result = cv2.medianBlur(frame, 11)

        # define a randomized grid of locations for the brush strokes
        grid = randomized_grid(frame.shape[0], frame.shape[1], scale=3)
        batch_size = 10000

        for h in range(0, len(grid), batch_size):
            # get the pixel colors at each point of the grid
            pixels = np.array([frame[x[0], x[1]] for x in grid[h:min(h + batch_size, len(grid))]])
