...
import pytest
from smart_canvas.core import CanvasCore, Painting, Idle, Startup, Active, ShowPic
...
finger_gun = cv2.resize(cv2.imread(f"{abs_file_path_finger}/finger_gun_test.png"), (1280, 720), interpolation = cv2.INTER_AREA)
...
class TestCoreState:
    def test_smart_canvas(self, core, queue):
        ...
        while type(core._state) is type(Painting()) and timeout < 100:
            print(f"painting")
            queue.put(face_neutral)
            timeout += 1
            time.sleep(0.1)
