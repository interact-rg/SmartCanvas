...
class TestFilters(object):
    def test_filter_with_image(self, carousel: FilterCarousel):
        frame = cv2.imread(f"tests/test_assets/small_image/image.png")
        mask = cv2.imread(f"tests/test_assets/small_image/mask.png")
        w, h, c = frame.shape
        for _ in carousel.catalog:
            filtered_frame = carousel.current_filter.filter_frame(frame, mask)
